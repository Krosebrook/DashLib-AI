...
const TemplateCard: React.FC<TemplateCardProps> = ({ template, onClick, isFavorite, onToggleFavorite }) => {
  ...
  const [templateReviews] = usePersistentState<Record<string, Review[]>>('dashlib-reviews', {});
  
  const currentReviews = templateReviews[template.id] || template.reviews || [];
  const averageRating = currentReviews.length > 0 
    ? currentReviews.reduce((acc, rev) => acc + rev.rating, 0) / currentReviews.length 
  ...
  return (
    ...
    >
      ...
      <h3 id={`title-${template.id}`} className="text-xl font-bold text-slate-900 mb-2 group-hover:text-indigo-600 transition-colors leading-tight pr-8">
        {template.title}
      </h3>
      ...
      {averageRating && (
        <div className="flex items-center gap-1 mb-3">
          <div className="flex text-amber-400">
            {[...Array(5)].map((_, i) => (
              <svg key={i} className={`w-3.5 h-3.5 ${i < Math.floor(averageRating) ? 'fill-current' : 'text-slate-200 fill-current'}`} viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
            ))}
          </div>
          <span className="text-xs font-bold text-slate-600">{averageRating.toFixed(1)}</span>
          <span className="text-[10px] text-slate-400">({currentReviews.length})</span>
        </div>
      )}
